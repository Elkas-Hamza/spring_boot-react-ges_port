...
const NavireForm = () => {
  ...
  useEffect(() => {
    ...
    const isRedirectingFromList = localStorage.getItem('redirecting_to_create_navire') === 'true';
    if (isRedirectingFromList) {
      ...
      setDebugInfo(`Form loaded via direct navigation. Mode: ${id ? 'Edit' : 'Create'}`);
    } else {
      ...
      const fetchNavire = async () => {
        try {
          ...
          if (response.success) {
            ...
            try {
              const containersResponse = await ConteneureService.getShipContainers(id);
              if (containersResponse.data) {
                setContainerIds(containersResponse.data.map(c => c.id_conteneure));
              }
            } catch (error) {
  ...
  const handleRemoveContainer = (containerId) => {
    ...
    setAllContainers(prev => prev.filter(container => container.id_conteneure !== containerId));
  };
      ...
      if (response.success) {
        ...
        for (const container of containersToProcess) {
          try {
            ...
            const creationResponse = await ConteneureService.createConteneure(containerData);
            ...
            if (!creationResponse || !creationResponse.data) {
              ...
              const containerResponse = await ConteneureService.createConteneure(basicContainerData);
              console.log("Created container with basic data:", containerResponse.data);
              
              // Now assign it to the ship
              await ConteneureService.assignContainerToShip(containerResponse.data.id_conteneure, shipId);
              console.log(`Assigned container ${containerResponse.data.id_conteneure} to ship ${shipId}`);
            }
            
            successCount++;
          } catch (error) {
            console.error(`Error processing container ${container.nom_conteneure}:`, error);
            errorCount++;
          }
        }
        ...
        setNotification({
          open: true,
          message: `Navire ${id ? 'modifié' : 'créé'} avec succès. ${successCount} conteneurs ajoutés.${errorCount > 0 ? ` ${errorCount} échecs.` : ''}`,
          severity: "success",
        });
        ...
        setDebugInfo(prev => `${prev}\nServer response: Error - ${response.message || 'Unknown error'}`);
        setNotification({
          open: true,
    ...
    <Container maxWidth="sm">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Box sx={{ mt: 4, mb: 2 }}>
            ...
            <Paper variant="outlined" sx={{ mt: 2, mb: 2 }}>
              <List>
                {containerIds.map((containerId) => {
                  const container = allContainers.find(c => c.id_conteneure === containerId);
                  return (
                    <ListItem
                      key={containerId}
                      secondaryAction={
                    ...
                    >
                      <ListItemText 
                        primary={container ? container.nom_conteneure : containerId} 
          ...
          {containerIds.length > 0 && (
            <Chip 
              label={`${containerIds.length} conteneur${containerIds.length > 1 ? 's' : ''}`} 
              color="primary" 
              variant="outlined" 
              size="small"
            />
      ...
      {process.env.NODE_ENV !== 'production' && (
        <Paper elevation={1} sx={{ mt: 4, p: 2, maxHeight: '200px', overflow: 'auto' }}>
